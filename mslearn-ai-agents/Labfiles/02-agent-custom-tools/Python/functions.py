...
def next_visible_event(location: str) -> str:
    today = int(datetime.now().strftime("%m%d")) # mes y dia convertido a entero "fechas a entero con en DAX"
    loc = location.lower().replace(" ", "-")

    for event_name, event_type, date, date_str, locs in EVENTS: # data load events
        if loc in locs and date >= today:
            return json.dumps({
                "event": event_name,
                "type": event_type,
                "date": date_str,
                "visible_from": sorted(locs)# Ordenado por locaciones
            })
    return json.dumps({"error": f"No upcoming events found for {location}."})


#notar que en esta funcion el if retornara un json tanto para el caso 1 y 0, se mantiene homogenea la salida (Mismo tipo de dato de salida)
# tip de programacion que la funcion haga solo una cosa "esta funcion solo devuelve un json", si se quiere despues decodificar posteriormente deberia ser en otra funcion .> principio de granuralidad o principio de responsabilidad unico como a los componentes "https://www.digitalocean.com/community/conceptual-articles/s-o-l-i-d-the-first-five-principles-of-object-oriented-design#open-closed-principle".
